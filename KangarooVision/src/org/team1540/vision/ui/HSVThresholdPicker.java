...
public class HSVThresholdPicker extends JComponent {
    ...
    private float minHue = 0.f, maxHue = 1.f;

    private float minSaturation = 0.f, maxSaturation = 1.f;

    private float minValue = 0.f, maxValue = 1.f;

    private int numHuePoints, numSaturationPoints, numValuePoints;

    private int saturationWidth;
    private int hueWidth;

    private int minHueBound = 70, maxHueBound = 180;

    private int minSaturationBound = 90, maxSaturationBound = 100;

    private int minValueBound = 60, maxValueBound = 100;
    ...
    public HSVThresholdPicker(int hueWidth, int saturationWidth, int valHeight) {
        ...
        this.addMouseMotionListener(new MouseMotionListener() {
            ...
            public void mouseMoved(MouseEvent e) {
                if (new Rectangle(minHueBound - 2, minValueBound - 2, 8, 8)
                        .contains(e.getX(),
                                e.getY())
                        || new Rectangle(maxHueBound - 2, maxValueBound - 2,
                                8, 8)
                                        .contains(e.getX(),
                                                e.getY())
    ...
    public void paint(Graphics g) {
        ...
        for (int s = 0; s < numSaturationPoints; ++s) {
            ...
            g2.setColor(Color.getHSBColor((getHueUpper() + getHueLower()) / 2, saturation,
        ...
        g2.setColor(Color.black);
        int[] xCoordinates = { hueWidth + saturationWidth + 1 + 1,
                hueWidth + saturationWidth + 1 + 1 + (int) (5 * Math.sqrt(3)),
                hueWidth + saturationWidth + 1 + 1 + (int) (5 * Math.sqrt(3)) };
        int[] yCoordinates = { minSaturationBound, minSaturationBound - 5, minSaturationBound + 5 };
        g2.fillPolygon(xCoordinates, yCoordinates, 3);

        int[] xCoordinates2 = { hueWidth + saturationWidth + 1 + 1,
                hueWidth + saturationWidth + 1 + 1 + (int) (5 * Math.sqrt(3)),
                hueWidth + saturationWidth + 1 + 1 + (int) (5 * Math.sqrt(3)) };
        int[] yCoordinates2 = { maxSaturationBound, maxSaturationBound - 5, maxSaturationBound + 5 };
