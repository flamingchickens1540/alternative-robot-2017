...
public class GoalBox {
    ...
    public double area() {
        return 0.5 * (Math.abs(
                determinant(bottomLeft.x, bottomLeft.y, 1, bottomRight.x, bottomRight.y, 1, topLeft.x, topLeft.y, 1))
                + Math.abs(determinant(bottomLeft.x, bottomLeft.y, 1, topRight.x, topRight.y, 1, topLeft.x, topLeft.y,
                        1)));
    }
    ...
    private static double determinant(double a, double b, double c, double d, double e, double f, double g, double h,
            double i) {
        return a * (e * i - f * h) - b * (d * i - f * g) + c * (d * h - e * g);
    }

    public static List<GoalBox> allGoalsInRectangle(Rect r, List<GoalBox> boxes) {
        return boxes.stream().filter(box -> r.contains(box.bottomLeft) && r.contains(box.bottomRight)
                && r.contains(box.topLeft) && r.contains(box.topRight)).collect(Collectors.toList());
    }
}
