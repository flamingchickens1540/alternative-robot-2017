...
public class Vision {
    ...
    /*
     * Given an image matrix and some HSV bounds, return a new matrix with a 1
     * in every location where the HSV of the pixel is within the bounds.
     */
    ...
    /*
     * Returns a double between 0 and 1 that represents how certain you are that
     * the given Mat is a thresholded picture of a goal.
     *
     * Algorithm: Pick the left-bottom-most, right-bottom-most, left-top-most,
     * right-top-most points. Draw what you think a goal should look like given
     * those vertices and compare the theoretical goal to the actual one. Return
     * the comparison result.
     */
    public static double goalCertainty(Mat input, Point leftBottomMost, Point leftTopMost, Point rightBottomMost,
            Point rightTopMost) {
        // accepting all matrices is actually a reasonable implementation for
        // now
        return 1.0f;
    }

    /*
     * Given the result of thresholdImage (a matrix of values either 0 or 1),
     * identify every solid image (a set of thresholded pixels that are all
     * adjacent.) This is a solid image iff the number of pixels in the set is
     * greater than or equal to minPixelSize.
     *
     * For all solid images within the larger matrix, pass them to
     * goalCertainty. If this returns a number greater than
     * GOAL_CERTAINTY_THRESHOLD, calculate the four points: left-top-most,
    ...
    public static List<GoalBox> isolateThresholdedGoals(Mat input, int minPixelSize) {
                ...
                if (marked.get(i, j)[0] < 0.5) {
                    if (input.get(i, j)[0] > 0.5) {
                        ...
                        while (!points.isEmpty()) {
                            ...
                            if (p_x + 1 < input.height() && input.get(p_x + 1, p_y)[0] > 0.5
                                    && marked.get(p_x + 1, p_y)[0] < 0.5) {
                                Point check = new Point(p_x + 1, p_y);
                                points.add(check);
                                marked.put(p_x + 1, p_y, 1);
                                if (bottomLeft.x - bottomLeft.y < check.x - check.y) {
                            ...
                            if (p_y + 1 < input.width() && input.get(p_x, p_y + 1)[0] > 0.5
                                    && marked.get(p_x, p_y + 1)[0] < 0.5) {
                                Point check = new Point(p_x, p_y + 1);
                                points.add(check);
                                marked.put(p_x, p_y + 1, 1);
                                if (bottomLeft.x - bottomLeft.y < check.x - check.y) {
                        ...
                        if (numPoints >= minPixelSize) {
                            stageOne.add(new GoalBox(new Point(bottomLeft.y, bottomLeft.x),
                                    new Point(bottomRight.y, bottomRight.x), new Point(topLeft.y, topLeft.x),
        ...
        // let them be about 50% off actual dimensions; might have to be
        // decreased
        double dimensionsSimilarity = 2.3;

        // check a rectangle twice the size (four times area)
        double rectMultiplier = 4.0;
        ...
        for (int i = 0; i < stageOne.size(); ++i) {
            ...
            Point centerToCheck = new Point(bCenter.x + unitSize * 9.95, bCenter.y);
            ...
            double[] rectPoints = { centerToCheck.x - rectMultiplier * unitSize * (2.0 / 2),
                    ...
                    centerToCheck.y + rectMultiplier * unitSize * (5.0 / 2), };
            Rect toCheckA = new Rect(new Point(rectPoints[0], rectPoints[1]), new Point(rectPoints[2], rectPoints[3]));
    ...
    /*
     * Calculate the horizontal angle away in degrees from the center given
     * anglePerPixel.
     */
